def sort_array(array):
    ...
    * sort_array([2, 4, 3, 0, 1, 5, 6]) => [6, 5, 4, 3, 2, 1, 0]
    """
    # To ensure order is stable in memory context we need to sort with 1-to 0th element
    # Otherwise, sorting with reverse would lose ordering.
    karno_array = numpy.abs(array)
    if not numpy.array:
        array = array[:]
        karno_array[:] = 1
    return array.view(karno_array.shape[0], -1)
