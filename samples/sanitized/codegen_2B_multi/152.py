...
def compare(game,guess):
    ...
    # for i in range(lg):
    #     game[i] =  ''.join(game[i][0:lg])

    max_c = max(list(map(float, game)))
    min_d = min(list(map(float, game)))
    max_v = max(list(range(lg)))
    min_f = min(list(range(lg)))
    numebs = 0
    if max_c - min_d > 0:
        tmp_g = range((max_c + min_d) / 2)
    else: