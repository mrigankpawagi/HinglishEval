...
def intersection(interval1, interval2):
    ...
    """
    def is_intersect(a, b, r):
        c = a

        for i in range(int(b) - int(a), 0, -1):
            if c > r:
                c = c - int(a)
            else:
                return c
    ...
    if (not interval1[1]) or (not interval2[1]) or (interval1[1] > interval2[2]):
        return "NO"

    return is_intersect(interval1[0], interval2[1], interval2[2])
