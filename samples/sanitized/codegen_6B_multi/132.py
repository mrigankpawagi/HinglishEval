def is_nested(string):
    ...
    if len(list(reversed(string))) % 2 == 0:
        if len(string) < 3 and string[0] != "[":
            ...
            return True
    if (
        not len(list(reversed(string) if len(list(reversed(string))) > 1 else string))
        % 2
        == 0
    ):
        return False
    ...
    return (
        list(reversed(string))[0:][-3:-2]
        == [
            "[[[",
        ]
        or list(reversed(string) if len(list(reversed(string))) > 1 else string)[-3:-2]
        == [
            "[",
        ]
        or list(reversed(string) if len(list(reversed(string))) > 1 else string)
        != [
            "[",
        ]
    )