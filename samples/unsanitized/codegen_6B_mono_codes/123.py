...
number_of_terms = int(input("Enter Number of terms to find : "))

# number = 8
# number_of_terms = 3

if(number > 0):
    sequence = get_odd_collatz(number)
    print(f"Collatz sequence : {sequence}")
    if (len(sequence) == number_of_terms):
        print(f'{number} is the {number_of_terms}th term in the Collatz Sequence.')
    else:
