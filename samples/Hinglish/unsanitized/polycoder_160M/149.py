...
def sorted_list_sum(lst):
    ...
    def _list_sort(me):
        ...
        for k, v in kares:
            result += [v]
        return [me.encode(c, "replace") for c, k in result]

    assert sorted_list_sum(lst) == sorted_list_sort(sorted_list(lst))  # Kokiri aura do karjis

    result = [sorted_list_sort(line) for line in lst]
    assert_array_equal(result, [0, 1])

    assert_array_equal(result, [0, None])  # List kar jias do karjis
