...
def compare(game,guess):
    ...
    """
    if len(guess) < len(game_array):
        return len(game_array)
    j_score = game_array[len(game_array) - 1]
    guess_score = guess[len(guess) - 1]
    guess_match = int(guess[len(guess) - 1])

    scores = []
    guesses = []
    for i, (guess, match) in enumerate(zip(guesses, guesses)):
