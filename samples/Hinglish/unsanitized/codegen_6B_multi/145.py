
def order_by_points(nums):
    """
    Ek function likho jo diye gaye integers ki list ko unke digits ka sum ke hisaab se ascending order mein sort kare.
    Dhyan do: agar kuch items ke digits ka sum similar ho, 
    toh unhe original list ke index ke basis par order karo.

    Jaise ki:
    >>> order_by_points([1, 11, -1, -11, -12]) == [-1, -11, 1, -12, 11]
    >>> order_by_points([]) == []
    """
    def _is_integer(num):
        try:
            int(num)
        except ValueError:
            return False
        return True

    def _sorter(x):
        """Sets order of given items in list by their decimal. """
        return [c for c in str(x) if _is_integer(c)]

    # Sort all items in the list
    sorted_nums = sorted(nums, key=_sorter)
    # Then, set order of the items in result
    return set(sorted_nums)
