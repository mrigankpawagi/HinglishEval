...
def fruit_distribution(s,n):
    ...
    """
    n = int(n)
    if sum(map(lambda x: x * s, map(int,input().split(" ")))) <= n:
        return n - sum(map(lambda x: x * s, map(int,input().split(" "))))
    elif sum(map(lambda x: x * s, map(int,input().split(" ")))) > n:
      return - float("inf")

if __name__ == '__main__':
    print("Enter size of basket : ", end = "")
    while True:
        s = input()
        if(is_string(s)):
            s = int(s)
          ...
          s = input()
    print("Basket has", end = "")
    print(s, "fruits ", end = "")
    print(fruit_distribution(s, s))
