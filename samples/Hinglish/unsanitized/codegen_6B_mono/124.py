...
def valid_date(date):
    ...
    try:
        day, month, year = [
            int(ch) for ch in date.strip().split("-",maxsplit=3)
        ]
        if 1 <= day <= 31:
            if month == 2 and (
                (year % 4 == 0 and day <= 29) or (year % 400 == 0 and day <= 29) or
                (year % 100 == 0 and day <= 28) or (day <= 28)
            ):
                return True
...
# print("test ::::",valid_date('03-11-2000'))
# print("test ::::",valid_date('15