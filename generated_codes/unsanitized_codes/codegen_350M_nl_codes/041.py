...
def car_race_collision(n: int):
    ...
    while True:
        if (car_race_collision(n-1)!= car_race_collision(n-1 + 1) or n_no > 0):
            break

    if car_race_collision(n_no)!= car_race_collision(n-1):
        return

    while (car_race_collision(n-1) == car_race_collision(n-1)):
        n_no += 1

    if (car_race_collision(n-1) == car_race_collision(n-1)):
        return

    while (car_race_collision(n) == car_race_collision(n-1)):
        n_no -= 1
