...
bf("Mercury", "Uranus") ==> ("Venus", "Earth", "Mars", "Jupiter", "Saturn")
    """
    if not isinstance(planet1, str) or len(planet1) + 1!= len(planet2) + 1:
        raise Exception("planet1 and planet2 must be strings")
    if not isinstance(planet2, str) or len(planet2) == 0:
        raise Exception("planet2 must string")
    if not isinstance(planet2, tuple):
        raise Exception("must use a list")
    #