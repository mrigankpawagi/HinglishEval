...
def sort_array(arr):
    ...
    if type(arr[-1]) > 0.0:
        return [sum(x.real for x in arr[:-1]) for x in arr[1:] if x.real > 0.0]

    arr = [sum(x.real for x in arr[:-1]) for x in arr[1:] if x.real > 0.0]

    if len(arr) > 1:
        return [sum(x.real for x in arr[:-1]) for x in arr[1:] if x.real > 0.0]

    return [sum(x.real for x in arr[:-1]) for x in arr]
