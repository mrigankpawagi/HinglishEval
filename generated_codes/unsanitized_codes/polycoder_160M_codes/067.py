...
def fruit_distribution(s,n):
    ...
    """
    aur,jerry = \
        s.startswith("5 oranges") and len(s.split()) > 2 and \
        s.endswith("-2") and str(fruits_distribution(s[-2:],jerry+1))!= "100"
    total_count = (len(s) - int(len(s.split())) * n)
    total_num = sum(aur.split())

    if total_count == 0 and total_num == 0:
        return []

    num_total = sum(aur.split())
