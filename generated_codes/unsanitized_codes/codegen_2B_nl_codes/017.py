...
def parse_music(music_string: str) -> List[int]:
    ...
    beat = 0
    for beat in range(9):
        if not musical_string[beat]: return notes
        if musical_string[beat].isupper():
            if musical_string[beat].isdigit():
                notes.append(int(musical_string[beat]))
            else:
                raise "Invalid character in musical note!"
        elif musical_string[beat].islower():
            if musical_string[beat].isdigit():
                notes.append(int(musical_string[beat]))
            else:
                raise "Invalid character in musical note!"
