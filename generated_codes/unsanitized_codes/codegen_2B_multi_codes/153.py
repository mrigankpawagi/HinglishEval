def Strongest_Extension(class_name, extensions):
    ...
    if class_name == "Extension":
        return ".".join(
            [class_name, str(Extensions.StrInExts(class_name)), class_name.upper()]
        )

    elif class_name not in extensions:
        return class_name

    else:  # aapko in extensions me inaak ne hain
        class_extensions = []
        for extension in extensions:
            if extension not in class_extensions:
                class_extensions.append(extension)
                class_extensions.sort(key=str)
