...
def split_words(txt):
    ...
    """
    return [i for i in txt.lower().replace(" ","").split(",") if i]
#    return "".join([i for i in txt.lower().replace(" ","").split(",") if i])
print split_words("Harek,sir,tareq,makki(kartik,piyush,sekar,krishna,neha,")
