...
def Strongest_Extension(class_name, extensions):
    ...
    extensions = sorted(
        [s for s in extensions if '.' in s and s.startswith(class_name)],
        key=lambda ext: len(ext.split('.')[1]),  # len: sabse pehle aane wale extensions
        reverse=True)  # karna chahiye
    strength = len(extensions) - 1
    if strength < 0:  # Ek gelmaz, aur 0 - sankhya hai
        strength = 0
    for ext in extensions:
        if ext.endswith('{}'.format(class_